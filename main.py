...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "https://lpezelier13-hue.github.io",
        "https://cfa-compagnons-troyes-v1.netlify.app",
        "http://localhost:3000",
        "http://127.0.0.1:5500",
    ],
...
# ── RÔLES ────────────────────────────────────────────────────────────────
ADMIN_ROLES = ['admin', 'responsable']
ALL_ROLES   = ['admin', 'responsable', 'formateur', 'secretariat', 'direction']

def is_admin(profil):
    return profil["role"] in ADMIN_ROLES

def can_create(profil):
    return profil["role"] in ['admin', 'responsable', 'formateur', 'secretariat']

def can_validate(profil):
    return profil["role"] in ADMIN_ROLES

def can_edit_trombi(profil):
    return profil["role"] in ['admin', 'responsable', 'secretariat']

def can_see_all(profil):
    return profil["role"] in ['admin', 'responsable', 'secretariat', 'direction']
...
class ApprentiUpdate(BaseModel):
...
def get_utilisateurs(profil=Depends(get_profil)):
    if not is_admin(profil):
        raise HTTPException(status_code=403, detail="Réservé à l'admin")
    res = supabase.table("profils").select("*").order("nom").execute()
    return res.data

@app.post("/utilisateurs")
def create_utilisateur(data: UtilisateurCreate, profil=Depends(get_profil)):
    if not is_admin(profil):
        raise HTTPException(status_code=403, detail="Réservé à l'admin")
    if data.role not in ALL_ROLES:
        raise HTTPException(status_code=400, detail=f"Rôle invalide. Valeurs acceptées : {ALL_ROLES}")
    try:
        # Créer le compte auth via sign_up
        ...
        if not auth_res.user:
            raise Exception("Erreur création compte auth")
        user_id = auth_res.user.id
        # Confirmer l'email directement via SQL
        supabase.rpc('confirm_user_email', {"user_id": user_id}).execute() if False else None
        # Créer le profil
        profil_res = supabase.table("profils").insert({
            "id": user_id,
            "email": data.email,
            "nom": data.nom,
            "prenom": data.prenom,
        ...
        return profil_res.data[0]
    except Exception as e:
        print(f"Create user error: {str(e)}")
        raise HTTPException(status_code=400, detail=str(e))

@app.put("/utilisateurs/{user_id}")
def update_utilisateur(user_id: str, data: UtilisateurUpdate, profil=Depends(get_profil)):
    if not is_admin(profil):
        raise HTTPException(status_code=403, detail="Réservé à l'admin")
    payload = {k: v for k, v in data.dict().items() if v is not None}
    res = supabase.table("profils").update(payload).eq("id", user_id).execute()
    return res.data[0]

@app.delete("/utilisateurs/{user_id}")
def delete_utilisateur(user_id: str, profil=Depends(get_profil)):
    if not is_admin(profil):
        raise HTTPException(status_code=403, detail="Réservé à l'admin")
    if user_id == profil["id"]:
        raise HTTPException(status_code=400, detail="Impossible de désactiver votre propre compte")
    # Désactiver uniquement dans profils (pas auth) — conserve l'historique
    supabase.table("profils").update({"actif": False}).eq("id", user_id).execute()
    return {"ok": True, "message": "Compte désactivé"}

@app.put("/utilisateurs/{user_id}/reactiver")
def reactiver_utilisateur(user_id: str, profil=Depends(get_profil)):
    if not is_admin(profil):
        raise HTTPException(status_code=403, detail="Réservé à l'admin")
    supabase.table("profils").update({"actif": True}).eq("id", user_id).execute()
...
def update_demande(demande_id: str, data: DemandeUpdate, profil=Depends(get_profil)):
    if not can_validate(profil):
        raise HTTPException(status_code=403, detail="Réservé au responsable ou admin")
...
def create_apprenti(data: ApprentiCreate, profil=Depends(get_profil)):
    if not can_edit_trombi(profil):
        raise HTTPException(status_code=403, detail="Non autorisé")
    aid = "E" + str(int(time.time()))[-6:]
    res = supabase.table("apprentis").insert({"id": aid, **data.dict()}).execute()
    return res.data[0]

@app.post("/apprentis/bulk")
def create_apprentis_bulk(data: list[ApprentiCreate], profil=Depends(get_profil)):
    if not can_edit_trombi(profil):
        raise HTTPException(status_code=403, detail="Non autorisé")
    payload = []
    for i, item in enumerate(data):
        aid = "E" + str(int(time.time()))[-6:] + str(i)
        payload.append({"id": aid, **item.dict()})
    res = supabase.table("apprentis").insert(payload).execute()
    return res.data

@app.put("/apprentis/{apprenti_id}")
def update_apprenti(apprenti_id: str, data: ApprentiUpdate, profil=Depends(get_profil)):
    if not can_edit_trombi(profil):
        raise HTTPException(status_code=403, detail="Non autorisé")
    payload = {k: v for k, v in data.dict().items() if v is not None}
    if not payload:
        raise HTTPException(status_code=400, detail="Aucune donnée à mettre à jour")
    res = supabase.table("apprentis").update(payload).eq("id", apprenti_id).execute()
    if not res.data:
...
# ── SAUVEGARDE COMPLÈTE ───────────────────────────────────────────────────
@app.get("/backup/full")
def get_full_backup(profil=Depends(get_profil)):
    if not is_admin(profil):
        raise HTTPException(status_code=403, detail="Réservé à l'admin")
    return {
        "date_export": datetime.datetime.now().isoformat(),
        "metiers": supabase.table("metiers").select("*").execute().data,
        "budgets": supabase.table("budgets").select("*").execute().data,
        "demandes": supabase.table("demandes").select("*").execute().data,
        "apprentis": supabase.table("apprentis").select("*").execute().data,
        "profils": supabase.table("profils").select("*").execute().data,
    }
